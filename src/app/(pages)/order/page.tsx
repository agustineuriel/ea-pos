...
import { Admin } from "@/components/admin/column";
import { OrderItem } from "@/components/order-item/column";
import { Order } from "@/components/order/column";
...
const CreateOrderPage = () => {
  ...
  const [newCustomerNumber, setNewCustomerNumber] = useState<string>("");
  ...
  const handleAddItem = () => {
    ...
    const itemToAdd = availableItems.find(
    ...
    if (quantity > itemToAdd.quantity) {
      setError(
        `Not enough stock. Available quantity: ${itemToAdd.quantity} ${itemToAdd.unit}`
      );
      return;
    }
    ...
    try {
      ...
      if (isCreatingNewCustomer) {
        ...
        if (!createCustomerResponse.ok) {
          ...
          throw new Error(
            customerErrorData.error || "Failed to create customer"
          );
        }
      ...
      for (const orderItem of orderItemsWithCorrectPrices) {
        ...
        if (!orderItemResponse.ok) {
          ...
          throw new Error(
            orderItemError.error || "Failed to create order item"
          );
        }
      }
      ...
      for (const orderItem of orderItemsWithCorrectPrices) {
        const itemToUpdate = availableItems.find(
          (item) => item.item_id === orderItem.item_id
        );
        if (itemToUpdate) {
          const newQuantity = itemToUpdate.quantity - orderItem.quantity;
          console.log(
            `Updating item ${orderItem.item_id} quantity to:`,
            newQuantity
          );
          const updateResponse = await fetch(
            `/api/quantity/${orderItem.item_id}`,
            {
              method: "PATCH",
              headers: {
                "Content-Type": "application/json",
          ...
          if (!updateResponse.ok) {
            ...
            throw new Error(
              updateErrorData.error ||
                `Failed to update quantity for item ${orderItem.item_id}`
            );
          }
        }
      }
    ...
    <div className="p-6">
        ...
        <div className="relative">
          <Input
            ...
            onChange={(e) => {
              ...
              // setNewCustomerName("Creating new customer");
            }}
            placeholder={
          ...
          {dropdownVisible && (
            <div
              className="absolute z-10 bg-white border border-gray-300 rounded-md mt-1 w-full max-h-40 overflow-y-auto"
              ...
              {customers
                .filter((customer) =>
                  customer.customer_name
                    .toLowerCase()
                    .includes(newCustomerName.toLowerCase())
                )
                .map((customer) => (
                  <div
                    ...
                    onClick={() => {
                      ...
                      setNewCustomerAddress(customer.customer_address);
                      setNewCustomerEmail(customer.customer_email);
                      setNewCustomerNumber(customer.customer_number || "");
                      setIsCreatingNewCustomer(false);
                      setDropdownVisible(false);
                    }}
                  >
      ...
      <div className="mb-4">
        ...
        <div className="flex items-end gap-4">
          ...
          <div className="flex-1">
            ...
            <Input type="number" value={price} className="w-full" readOnly />
          </div>
          <Button
      ...
      <div className="flex gap-4">
        <Button
          ...
          className={
            loading || orders.length > 0
              ? "opacity-50 cursor-not-allowed border-1"
              : "border-1"
          }
        >
        ...
        {orders.length > 0 && (
          <Button
            onClick={() => {
              window.location.href = "/transaction-history"; // Replace with your desired page path
            }}
            className="border-1"
          >
        ...
        {orders.length > 0 && (
          <Button
            onClick={() => {
              if (orders.length > 0) {
                const orderId = orders[0].order_id; // Assuming the first order is the one to generate an invoice for
                window.location.href = `/invoice/${orderId}`; // Pass the order ID as a query parameter
              }
            }}
            className="border-1"
          >
